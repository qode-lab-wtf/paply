...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  sendMicCaptureStarted: (d) => ipcRenderer.send('meeting:mic-capture-started', d),
  ...
  captureFlushed: () => ipcRenderer.invoke('meeting:capture-flushed'),
  ...
  reanalyzeMeeting: (id) => ipcRenderer.invoke('meetings:reanalyze', id),
  ...
  exportMeeting: (payload) => ipcRenderer.invoke('meetings:export', payload),
  // Listener liefern eine Abmelde-Funktion (React-Effekte räumen auf, keine doppelten Handler).
  onMeetingStatus: (cb) => { const h = (_e, s) => cb(s); ipcRenderer.on('meeting:status', h); return () => ipcRenderer.removeListener('meeting:status', h); },
  onMeetingStarted: (cb) => { const h = (_e, d) => cb(d); ipcRenderer.on('meeting:started', h); return () => ipcRenderer.removeListener('meeting:started', h); },
  onMeetingStopped: (cb) => { const h = (_e, d) => cb(d); ipcRenderer.on('meeting:stopped', h); return () => ipcRenderer.removeListener('meeting:stopped', h); },
  onMeetingCallState: (cb) => { const h = (_e, d) => cb(d); ipcRenderer.on('meeting:call-state', h); return () => ipcRenderer.removeListener('meeting:call-state', h); },
  onMeetingCaptureStop: (cb) => { const h = (_e, d) => cb(d); ipcRenderer.on('meeting:capture-stop', h); return () => ipcRenderer.removeListener('meeting:capture-stop', h); },
  onMeetingsUpdated: (cb) => { const h = (_e, d) => cb(d); ipcRenderer.on('meetings:updated', h); return () => ipcRenderer.removeListener('meetings:updated', h); },
